...
async function updateLinkedInPostStatus(id: string) {
  ...
  if (updateError) {
    ...
    return {
      error:
        "Post published to LinkedIn, but failed to update status in Supabase",
  ...
  if (!updated || updated.length === 0) {
    ...
    if (genError) {
      ...
      return {
        error:
          "Post published to LinkedIn, but failed to update status in generations",
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      "[LINKEDIN PUBLISH] externalAccounts:",
    ...
    console.log(
      "[LINKEDIN PUBLISH] linkedinAccount:",
      JSON.stringify(linkedinAccount)
    );

    if (!linkedinAccount) {
      return NextResponse.json(
        {
          error: "No LinkedIn connection found in Clerk.",
    ...
    const linkedinProvider = linkedinAccount.provider as
      | "oauth_linkedin"
      | "oauth_linkedin_oidc";
      ...
      return NextResponse.json(
        {
          ...
          debug: { userId, provider: linkedinProvider, error: String(error) },
        },
        { status: 500 }
      );
    }
    ...
    console.log(
      "[LINKEDIN PUBLISH] tokensResponse:",
    ...
    const linkedinUserId: string | null | undefined =
      linkedinAccount.externalId || linkedinAccount.username;
    ...
    if (imageUrl) {
      try {
        ...
        const result = await uploadLinkedInImage(
          accessToken,
          linkedinUserId,
          imageUrl,
          imageAlt
    ...
    if (id) {
      const statusError = await updateLinkedInPostStatus(id);
      if (statusError) {
        return NextResponse.json(
          {
            ...statusError,
